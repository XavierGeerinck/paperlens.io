...
const GameOfLife: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initGrid = () => {
      ...
      const newGrid = new Array(cols * rows).fill(0).map(() => (Math.random() > spawnRate ? 1 : 0));
    ...
    const draw = () => {
      ...
      // Grid Lines
      ctx.strokeStyle = isOverclocked ? 'rgba(34, 197, 94, 0.1)' : 'rgba(255, 255, 255, 0.01)';
      ctx.lineWidth = 1;
      ctx.beginPath();
      for (let x = 0; x <= cols; x++) {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 z-[-1] overflow-hidden bg-[#09090b] pointer-events-none">
        <canvas 
            ref={canvasRef} 
