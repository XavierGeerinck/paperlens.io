...
const SEALAdaptationSimulation: React.FC = () => {
	...
	const { isRunning, state, history, epoch, start, stop, reset } =
		useSimulation<SimulationState>({
			...
			onTick: (prev, tick) => {
				...
				if (tick % 10 === 0) {
					...
					const edits: SelfEdit[] = Array.from(
						{ length: prev.numEdits },
						(_, i) => {
							...
							const noisyReward = Math.max(
								0,
								Math.min(1, baseReward + (Math.random() - 0.5) * 0.2),
							);
					...
					const avgReward =
						selectedEdits.reduce((sum, e) => sum + e.reward, 0) /
					...
					<div className="space-y-4">
						...
						<div className="bg-slate-900/50 rounded-lg p-3 border border-slate-700/50">
							...
							<ul className="text-xs text-slate-300 space-y-1 list-disc list-inside">
								<li>
									<strong>Green particles</strong>: High-reward self-edits
									selected for RL update
								</li>
								<li>
									<strong>Policy Accuracy</strong>: Improves as better edits are
									discovered
								</li>
								<li>
									<strong>Knowledge Retention</strong>: Decreases due to
									catastrophic forgetting
								</li>
								<li>
									<strong>Top-K</strong>: Higher K = more diversity but slower
									convergence
								</li>
							</ul>
						</div>
